...
router.post("/workout-plan", async (req, res) => {
  ...
  if (!profile?.height_cm || !profile?.weight_kg)
    return res
      .status(400)
      .json({ error: "Complete your health profile first." });
  ...
  if (!apiKey)
    return res
      .status(500)
      .json({ error: "AI not configured. Add GROQ_API_KEY to env." });

  const effectiveGoal = goal || profile.health_goal || "general";
  const age = profile.date_of_birth
    ? Math.floor(
        (Date.now() - new Date(profile.date_of_birth)) /
          (365.25 * 24 * 3600 * 1000),
      )
    : 30;
  const conditions =
  ...
  const goalDescriptions = {
    ...
    weight_loss:
      "Fat loss — HIIT, calorie-burning circuits, strength to preserve lean mass",
    diabetes:
      "Diabetic-friendly — low impact, post-meal walks, resistance bands, blood sugar management",
    heart:
      "Cardiac health — Zone 2 cardio, low-intensity steady state, light resistance, breathing exercises",
    general:
      "General wellness — balanced full body, cardio, flexibility, active recovery",
  };
...
Requirements: exactly 7 days, Day 7 = active recovery, 3-6 exercises/day, sets=number, reps=string, rest=string, 4 nutrition tips specific to the goal.`;
  ...
  try {
    ...
    res
      .status(500)
      .json({ error: "Failed to generate workout plan. " + err.message });
  }
});
