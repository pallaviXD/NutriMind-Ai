  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (attempt < maxRetries) {
        ...
        console.warn(
          `Groq attempt ${attempt}/${maxRetries} failed (${status || "network"}), retrying in ${delay}ms…`,
        );
...
router.get("/stats", async (req, res) => {
  ...
  if (!profile?.height_cm || !profile?.weight_kg)
    return res
      .status(400)
      .json({ error: "Set height and weight in your profile first." });
  ...
  const age = date_of_birth
    ? Math.floor(
        (Date.now() - new Date(date_of_birth)) / (365.25 * 24 * 3600 * 1000),
      )
    : 30;
...
router.post("/weight-log", async (req, res) => {
  ...
  await db
    .prepare(
      "INSERT INTO weight_logs (user_id, weight_kg, notes) VALUES (?, ?, ?)",
    )
    .run(req.user.id, weight_kg, notes || null);
  await db
    .prepare("UPDATE profiles SET weight_kg = ? WHERE user_id = ?")
    .run(weight_kg, req.user.id);
...
router.post("/chat", async (req, res) => {
  ...
  const calsLeft = Math.max(
    0,
    (calories?.target || 2000) - (calories?.current || 0),
  );
  const nextSlot =
  ...
  const userContext = [
    ...
    profile?.date_of_birth
      ? `Age: ${Math.floor((Date.now() - new Date(profile.date_of_birth)) / (365.25 * 24 * 3600 * 1000))} years`
      : "",
...
function getGoalGuidance(goal) {
  switch (goal) {
    case "gym":
      return "prioritize protein timing, muscle recovery, caloric surplus";
    case "diabetes":
      return "warn about high-GI foods, suggest low-GI alternatives, mention blood sugar";
    case "weight_loss":
      return "encourage deficit, high protein for satiety, avoid liquid calories";
    case "heart":
      return "low sodium, omega-3 rich foods, avoid saturated fats, DASH diet";
    default:
      return "balanced practical advice, whole foods, hydration, sleep";
  }
}
    ...
    if (!recipe)
      return res
        ...
        .json({
          success: false,
          errorCode: "AI_INVALID_RESPONSE",
          error: "AI returned an invalid recipe. Please try again.",
        });
    res.json({
      success: true,
      recipe,
      meta: { providerLatencyMs: Date.now() - startTime },
    });
  } catch (err) {
...
router.post("/health-advice", async (req, res) => {
  ...
  const age = details.date_of_birth
    ? Math.floor(
        (Date.now() - new Date(details.date_of_birth)) /
          (365.25 * 24 * 3600 * 1000),
      )
    : null;
  const bmi =
    details.height_cm && details.weight_kg
      ? (details.weight_kg / (details.height_cm / 100) ** 2).toFixed(1)
      : null;
...
router.post("/meal-log", async (req, res) => {
  ...
  if (!food_name || !calories)
    return res.status(400).json({ error: "food_name and calories required." });
  await db
    .prepare(
      "INSERT INTO meal_logs (user_id, food_name, calories, protein, carbs, fat, meal_type) VALUES (?, ?, ?, ?, ?, ?, ?)",
    )
    .run(
      req.user.id,
      food_name,
...
router.get("/meal-log", async (req, res) => {
  const logs = await db
    .prepare(
      "SELECT * FROM meal_logs WHERE user_id = ? ORDER BY logged_at DESC LIMIT 50",
    )
    .all(req.user.id);
  res.json({ logs });
});
...
router.get("/analytics", async (req, res) => {
  ...
  const [dailyTotals, weightLogs, todayByMeal, totalMealsRow, recentDays] =
    await Promise.all([
      db
        .prepare(
          `
    ...
    `,
        ...
        .all(req.user.id),
      db
        .prepare("SELECT COUNT(*) as count FROM meal_logs WHERE user_id = ?")
        .get(req.user.id),
      db
        .prepare(
          `
  ...
  if (recentDays.length > 0) {
    ...
    const yesterday = new Date(Date.now() - 86400000)
      ...
      .split("T")[0];
    let expectedTime =
      recentDays[0].day === today
        ? new Date(today).getTime()
        : recentDays[0].day === yesterday
          ? new Date(yesterday).getTime()
          : null;
    if (expectedTime !== null) {
      for (const row of recentDays) {
        if (row.day === new Date(expectedTime).toISOString().split("T")[0]) {
          streak++;
  ...
  res.json({
    ...
    weightLogs,
    todayByMeal,
    totalMeals: totalMealsRow?.count ?? 0,
    streak,
  });
});
...
router.post("/water-log", async (req, res) => {
  ...
  if (glasses === undefined || glasses === null)
    return res.status(400).json({ error: "glasses is required." });
  const today = new Date().toISOString().split("T")[0];
  await db
    .prepare(
      `
