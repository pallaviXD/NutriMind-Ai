...
const Sidebar = ({ onLogout, isExpanded = true }) => {
  ...
  return (
    <div
      className={`w-16 ${isExpanded ? "lg:w-60" : "lg:w-16"} h-full bg-panel/80 backdrop-blur-xl border-r border-border flex flex-col transition-all duration-300 shrink-0 z-20`}
      role="navigation"
      aria-label="Main navigation"
    >
      {/* Brand */}
      <div
        className={`h-14 flex items-center justify-center ${isExpanded ? "lg:justify-start lg:px-5" : ""} border-b border-border/50 shrink-0`}
      >
        <Activity size={24} className="text-accent-neon shrink-0" />
        <span
          className={`${isExpanded ? "hidden lg:block" : "hidden"} ml-2.5 font-bold text-base tracking-tight`}
        >
      ...
      <div className="p-3 border-t border-border/50 space-y-2">
        {healthProfile && (
          <div
            className={`${isExpanded ? "hidden lg:flex" : "hidden"} items-center gap-2 px-2 py-1.5 rounded-xl bg-accent-neon/10 border border-accent-neon/20`}
          >
            <HeartPulse size={12} className="text-accent-neon shrink-0" />
            <span className="text-[10px] font-semibold text-accent-neon truncate capitalize">
        ...
        >
          ...
          <span
            className={`${isExpanded ? "hidden lg:block" : "hidden"} font-medium`}
          >
            Sign Out
          </span>
        </button>
        ...
        <>
          {isActive && (
            <FramerMotion.div
              ...
              className="absolute left-0 top-1 bottom-1 w-0.5 bg-accent-neon rounded-r-full"
              initial={false}
          ...
          <span
            className={`${isExpanded ? "hidden lg:block" : "hidden"} ml-3 text-sm truncate`}
          >
            {label}
          </span>
        </>
