...
const Kitchen = () => {
  const { pantry, setPantry, macros, calories, healthProfile } =
    useGlobalState();
      ...
      <div className="w-full lg:w-1/3 flex flex-col gap-4 min-h-0">
        <div className="glass-panel p-5 flex flex-col flex-1 overflow-hidden">
          ...
          <form onSubmit={handleAddItem} className="flex gap-2 mb-4 shrink-0">
            ...
            <button
              type="submit"
              className="p-2 bg-accent-cyan/20 text-accent-cyan hover:bg-accent-cyan/30 rounded-xl transition-colors"
            >
              <Plus size={18} />
            </button>
          </form>
          ...
          <div className="flex-1 overflow-y-auto space-y-2 pr-1">
            {pantry.map((item, idx) => (
              <div
                key={idx}
                className="flex items-center justify-between px-3 py-2.5 rounded-xl bg-background/50 border border-border/50 group hover:border-accent-cyan/30 transition-colors"
              >
          ...
          <div>
            ...
            <div className="flex gap-4 text-sm">
              <span className="text-accent-neon font-bold">
                {Math.max(0, calories.target - calories.current)} kcal
              </span>
              <span className="text-accent-cyan font-bold">
                {Math.max(0, macros.targetProtein - macros.protein)}g protein
              </span>
              <span className="text-accent-purple font-bold">
                {Math.max(0, macros.targetCarbs - macros.carbs)}g carbs
              </span>
            </div>
          </div>
          <button
          ...
          <AnimatePresence mode="wait">
            {loading && (
              ...
              >
                ...
                <p className="text-accent-purple animate-pulse text-sm">
              ...
              >
                ...
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <h2 className="text-2xl font-bold text-foreground flex items-center gap-2">
                      <ChefHat size={24} className="text-accent-purple" />{" "}
                      {recipe.name}
                    </h2>
                ...
                <div className="grid grid-cols-4 gap-3">
                  {[
                    ...
                    >
                      <div
                        className={`flex items-center justify-center gap-1 ${s.color} mb-1`}
                      >
                        {s.icon}
                      </div>
                ...
                <div>
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    {recipe.ingredients?.map((ing, i) => (
                      <div
                        key={i}
                        className="flex items-center gap-2 text-sm bg-background/30 rounded-lg px-3 py-2 border border-border/30"
                      >
                ...
                {recipe.tips && (
                  <div className="bg-accent-neon/5 border border-accent-neon/20 rounded-xl p-4">
                    <p className="text-xs font-semibold text-accent-neon uppercase tracking-wider mb-1">
                      💡 Chef's Tip
                    </p>
            ...
            {!recipe && !loading && !error && (
              ...
              >
                ...
                <p className="text-muted text-sm">
                  Add ingredients to your pantry and click
                  <br />
                  <span className="text-accent-purple font-semibold">
                    Generate Recipe
                  </span>{" "}
                  to get a personalized meal
                </p>
              </motion.div>
            )}
          </AnimatePresence>
