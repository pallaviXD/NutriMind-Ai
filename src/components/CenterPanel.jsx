    ...
    <div className="flex flex-col gap-6 h-full overflow-y-auto pr-2 pb-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        ...
        <div className="glass-panel p-6 flex flex-col items-center justify-center relative overflow-hidden group">
          ...
          <h3 className="text-sm text-muted font-medium mb-4 uppercase tracking-wider">
            Daily Completion
          </h3>
          ...
          <div className="relative w-32 h-32 flex items-center justify-center">
            ...
            >
              ...
              <motion.circle
                ...
                animate={{
                  strokeDasharray: `${(dailyCompletion / 100) * 283} 283`,
                }}
        ...
        <div className="md:col-span-2 glass-panel p-6 flex flex-col justify-center relative overflow-hidden group">
          ...
          <div className="h-4 bg-background rounded-full overflow-hidden relative border border-border/50">
            <motion.div
              ...
              initial={{ width: 0 }}
              animate={{
                width: `${Math.min((calories.current / calories.target) * 100, 100)}%`,
              }}
      ...
      <div className="glass-panel p-6 flex-1 min-h-[250px] flex flex-col">
        <h3 className="text-sm text-muted font-medium uppercase tracking-wider mb-4">
          ...
          <ResponsiveContainer width="100%" height="100%">
            ...
            >
              ...
              <Bar dataKey="value" radius={[4, 4, 0, 0]}>
                {[meals.breakfast, meals.lunch, meals.dinner, meals.snacks].map(
                  (entry, index) => (
                    <Cell
                      key={`cell-${index}`}
                      fill={entry.status === "skipped" ? "#27272a" : "#8b5cf6"}
                    />
                  ),
                )}
              </Bar>
...
const MealCard = ({ type, data, isAnalyzing }) => {
  const getStatusIcon = () => {
    if (data.status === "completed")
      return <CheckCircle2 size={18} className="text-accent-cyan" />;
