...
const Analytics = () => {
  ...
  const avgCalories = hasData
    ? Math.round(
        data.dailyTotals.reduce((s, d) => s + d.calories, 0) /
          data.dailyTotals.length,
      )
    : 0;
  const avgProtein = hasData
    ? Math.round(
        data.dailyTotals.reduce((s, d) => s + d.protein, 0) /
          data.dailyTotals.length,
      )
    : 0;
  const targetCals =
    profile?.health_goal === "gym"
      ? 2800
      : profile?.health_goal === "weight_loss"
        ? 1600
        : 2200;
  ...
  return (
    ...
    >
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 shrink-0">
        ...
        />
        <StatCard
          icon={<Zap size={18} className="text-accent-neon" />}
          label="Avg Protein"
          value={avgProtein || "—"}
          unit="g/day"
          color="border-l-accent-neon"
        />
        <StatCard
        ...
        />
        <StatCard
          icon={<Calendar size={18} className="text-accent-purple" />}
      ...
      {!hasData ? (
        <div className="flex-1 flex flex-col items-center justify-center gap-4 text-center">
          ...
          <div>
            ...
            <p className="text-muted text-sm mt-1">
              Start logging meals in the chat to see your analytics here
            </p>
          </div>
        </div>
      ) : (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 flex-1 min-h-0">
          {/* Calorie trend */}
          <div
            className="glass-panel p-5 flex flex-col h-[320px]"
            role="img"
          ...
          <div
            className="glass-panel p-5 flex flex-col h-[320px]"
            role="img"
            aria-label="Macro breakdown chart"
          >
            <h2 className="text-sm font-bold uppercase tracking-wider text-muted mb-4 flex items-center gap-2">
          ...
          {data.weightLogs?.length > 0 && (
            <div
              className="glass-panel p-5 flex flex-col h-[280px]"
              role="img"
              aria-label="Weight trend chart"
            >
              <h2 className="text-sm font-bold uppercase tracking-wider text-muted mb-4 flex items-center gap-2">
                <TrendingUp size={14} className="text-accent-purple" /> Weight
                Trend
              </h2>
              <div className="flex-1">
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart
                    data={[...data.weightLogs].reverse()}
                    margin={{ top: 5, right: 10, left: -20, bottom: 0 }}
                  >
                    <CartesianGrid
                      strokeDasharray="3 3"
          ...
          {data.todayByMeal?.length > 0 && (
            <div
              className="glass-panel p-5 flex flex-col h-[280px]"
              role="img"
              aria-label="Today's meals chart"
            >
              <h2 className="text-sm font-bold uppercase tracking-wider text-muted mb-4 flex items-center gap-2">
                <Calendar size={14} className="text-accent-cyan" /> Today by
                Meal
              </h2>
              <div className="flex-1">
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart
                    data={data.todayByMeal}
                    margin={{ top: 5, right: 10, left: -20, bottom: 0 }}
                  >
                    <CartesianGrid
                      strokeDasharray="3 3"
...
const StatCard = ({ icon, label, value, unit, color }) => (
  <div
    className={`glass-panel p-4 border-l-4 ${color} flex items-center gap-3`}
    role="article"
  >
