...
const QUICK_ACTIONS = [
  {
    key: "skipped_meal",
    label: "⏭ Skipped Meal",
    color: "hover:border-yellow-500/50 hover:text-yellow-400",
  },
  {
    key: "ate_junk",
    label: "🍕 Ate Junk",
  ...
  },
  {
    key: "high_protein",
    label: "💪 High Protein",
    color: "hover:border-accent-purple/50 hover:text-accent-purple",
  },
];
...
const LeftPanel = () => {
  const { chatHistory, isAnalyzing, processUserInput, calories, macros } =
    useGlobalState();
      ...
      <div className="p-4 border-b border-border/50 shrink-0">
        ...
        {user && (
          <p className="text-xs text-muted mt-0.5">
            ...
            {profile?.health_goal
              ? `${profile.health_goal.replace("_", " ")} mode`
              : "tell me anything"}
          </p>
        )}
      </div>
        ...
        {chatHistory.map((msg, idx) => {
          ...
          if (isSystem)
            return (
              <div
                key={idx}
                className="flex items-start gap-2 text-xs text-muted bg-border/20 rounded-xl px-3 py-2"
              >
        ...
        {isAnalyzing && (
          ...
          >
            ...
            <div className="px-3 py-2.5 rounded-2xl rounded-bl-sm bg-panel border border-border text-xs flex items-center gap-1">
              <span
                className="w-1.5 h-1.5 rounded-full bg-accent-neon animate-bounce"
...
function formatMessage(text) {
  ...
  return parts.map((part, i) =>
    i % 2 === 1 ? (
      <strong key={i} className="text-accent-neon font-semibold">
        {part}
      </strong>
    ) : (
      part
