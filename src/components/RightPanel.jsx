...
const RightPanel = ({ state, isAnalyzing }) => {
  ...
  const getRiskColor = (level) => {
    switch (level) {
      case "High":
        return "text-red-500 shadow-[0_0_15px_rgba(239,68,68,0.5)] border-red-500/50";
      case "Medium":
        return "text-yellow-500 shadow-[0_0_15px_rgba(234,179,8,0.3)] border-yellow-500/50";
      case "Low":
        return "text-accent-neon shadow-[0_0_15px_rgba(14,165,233,0.3)] border-accent-neon/50";
      default:
  ...
  return (
    <div className="flex flex-col gap-4 h-full overflow-y-auto pr-2 pb-4">
      {/* Risk Meter */}
      <div
        className={`glass-panel p-4 flex items-center justify-between border transition-colors duration-500 ${getRiskColor(riskLevel)}`}
      >
        ...
        <div className="flex items-center gap-2">
          ...
          {isAnalyzing && (
            <div className="w-2 h-2 rounded-full bg-current animate-ping ml-2" />
          )}
        </div>
      </div>
      ...
      <div className="glass-panel p-5 flex flex-col flex-none h-[220px]">
        <h3 className="text-xs text-muted font-semibold uppercase tracking-wider mb-4 flex items-center gap-2">
          <TrendingUp size={14} className="text-accent-neon" /> Consistency
          Trend
        </h3>
      ...
      <div className="glass-panel p-5 flex flex-col flex-1 min-h-[220px]">
        <h3 className="text-xs text-muted font-semibold uppercase tracking-wider mb-2 flex items-center gap-2">
          <PieChartIcon size={14} className="text-accent-purple" /> Macro
          Distribution
        </h3>
            ...
            <PieChart>
              ...
              >
                {charts.macros.map((entry, index) => (
                  <Cell
                    key={`cell-${index}`}
                    fill={["#0ea5e9", "#8b5cf6", "#06b6d4"][index % 3]}
                  />
                ))}
              </Pie>
              <Tooltip
        ...
        <div className="flex items-center gap-2 mb-2">
          <div className="flex gap-1 flex-wrap flex-1">
            {Array.from({ length: 8 }).map((_, i) => (
              <div
                key={i}
                className={`w-5 h-5 rounded-full border transition-all ${i < water ? "bg-blue-400 border-blue-400" : "border-border"}`}
