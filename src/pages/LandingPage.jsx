...
import {
  Activity, Brain, ChefHat, BarChart3, Dumbbell, Droplets,
  MessageSquare, Zap, Shield, ArrowRight, CheckCircle2,
  Flame, Heart, Pill, User, ChevronDown, Star, Sparkles
} from 'lucide-react';
...
const GOALS = [
  { icon: <Dumbbell size={20} />, label: 'Gym & Muscle',   desc: 'High protein, caloric surplus',     color: 'text-blue-400',    bg: 'bg-blue-500/10 border-blue-500/30' },
  { icon: <Activity size={20} />, label: 'Weight Loss',    desc: 'Caloric deficit, fat-loss mode',   color: 'text-orange-400',  bg: 'bg-orange-500/10 border-orange-500/30' },
  { icon: <Pill size={20} />,     label: 'Diabetes Care',  desc: 'Low-GI, blood sugar management',  color: 'text-emerald-400', bg: 'bg-emerald-500/10 border-emerald-500/30' },
  { icon: <Heart size={20} />,    label: 'Cardiac Health', desc: 'Low sodium, DASH diet',            color: 'text-pink-400',    bg: 'bg-pink-500/10 border-pink-500/30' },
  { icon: <User size={20} />,     label: 'General Wellness', desc: 'Balanced everyday nutrition',   color: 'text-purple-400',  bg: 'bg-purple-500/10 border-purple-500/30' },
];

const CHAT_DEMO = [
  { role: 'user', text: 'I had 2 rotis with dal and a glass of buttermilk for lunch' },
  { role: 'ai',  text: '✅ Logged **Roti + Dal + Buttermilk** → ~**485 kcal** added to Lunch\n\n• Protein: +18g  • Carbs: +72g  • Fat: +9g\n\n**1,215 / 2,000 kcal** consumed today. You\'re on track — great balanced meal! 💪' },
  { role: 'user', text: 'What should I have for dinner given I want to hit my protein goal?' },
  { role: 'ai',  text: '🎯 You need **~52g more protein** tonight. I\'d suggest:\n\n• **Grilled chicken breast** (200g) — 46g protein, 330 kcal\n• Pair with **steamed broccoli + brown rice** for complete macros\n\nWant me to generate a full recipe from your pantry? 🍽️' },
];
    ...
    >
      <div className="max-w-7xl mx-auto px-6 h-16 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-8 text-sm text-muted">
          ...
          <a href="#goals" className="hover:text-foreground transition-colors">Health Goals</a>
        </div>
        <div className="flex items-center gap-3">
          <Link to="/login" className="text-sm text-muted hover:text-foreground transition-colors px-4 py-2">
            Sign in
          </Link>
          <Link to="/signup" className="text-sm font-semibold bg-gradient-to-r from-accent-neon to-accent-purple text-white px-5 py-2 rounded-full hover:shadow-[0_0_20px_rgba(139,92,246,0.4)] transition-all">
            Get Started Free
          </Link>
        </div>
      </div>
  ...
  return (
    <div className="min-h-screen bg-background text-foreground overflow-x-hidden">
      ...
      <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
        ...
        <motion.div style={{ opacity: heroOpacity }} className="relative z-10 text-center px-6 max-w-5xl mx-auto pt-16">
          {/* Badge */}
          <motion.div
          ...
          >
            Log meals in plain language, get AI-generated recipes from your pantry,
            track real nutrition data, and receive personalized health advice — all
            tuned to your body and health conditions.
          </motion.p>
      ...
      <section id="how-it-works" className="py-24 px-6 relative overflow-hidden">
        ...
        <div className="max-w-5xl mx-auto relative">
          ...
          >
            ...
            <h2 className="text-3xl md:text-5xl font-black mb-4">Just talk to it</h2>
            <p className="text-muted max-w-xl mx-auto">No forms, no manual entry. Chat naturally and the AI handles the rest.</p>
          </motion.div>
          ...
          <div className="grid md:grid-cols-2 gap-12 items-center">
            ...
            >
              <div className="flex items-center gap-2 pb-3 border-b border-border">
                ...
                <span className="text-xs font-semibold text-accent-neon">NutriMind AI</span>
            ...
            >
              {[
                { n: '01', title: 'Set up your profile', desc: 'Enter height, weight, age, activity level, and health conditions. Takes 60 seconds.' },
                { n: '02', title: 'Chat naturally',       desc: 'Tell the AI what you ate, ask for recipes, request health advice — in any language.' },
                { n: '03', title: 'Watch your dashboard update', desc: 'Calories, macros, meal cards, and insights update in real time as you chat.' },
                { n: '04', title: 'Track your progress',  desc: 'Analytics show real trends from your data. Weight, streaks, macro history — all yours.' },
              ].map((step, i) => (
                <motion.div
                  key={step.n}
                  initial={{ opacity: 0, y: 12 }}
      ...
      <section id="features" className="py-24 px-6 bg-panel/20">
        <div className="max-w-6xl mx-auto">
          ...
          >
            ...
            <h2 className="text-3xl md:text-5xl font-black mb-4">One platform, total health clarity</h2>
            <p className="text-muted max-w-xl mx-auto">Every feature is connected — AI chat updates your analytics, your profile shapes AI responses, your pantry drives your recipes.</p>
          </motion.div>
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-5">
            {FEATURES.map((f, i) => (
              ...
              >
                <div className={`w-10 h-10 rounded-xl bg-background/60 flex items-center justify-center mb-4 ${f.accent}`}>
                  {f.icon}
                </div>
      ...
      <section id="goals" className="py-24 px-6">
        <div className="max-w-4xl mx-auto">
          ...
          >
            ...
            <h2 className="text-3xl md:text-5xl font-black mb-4">Not a generic calorie counter</h2>
            <p className="text-muted max-w-xl mx-auto">NutriMind adapts entirely to your health goal — different calorie targets, macro ratios, food restrictions, and AI guidance for each.</p>
          </motion.div>
      ...
      <section className="py-12 px-6 border-y border-border bg-panel/30">
        <div className="max-w-4xl mx-auto flex flex-wrap justify-center gap-8 text-sm text-muted">
          {[
            ...
            { icon: <CheckCircle2 size={15} />, text: 'No API keys in the browser' },
            { icon: <Star size={15} />,   text: 'Parameterized SQL queries' },
          ].map((item) => (
            <div key={item.text} className="flex items-center gap-2 text-muted/70">
      ...
      <footer className="border-t border-border py-8 px-6">
        <div className="max-w-6xl mx-auto flex flex-col md:flex-row items-center justify-between gap-4 text-xs text-muted">
          ...
          <div className="flex items-center gap-6">
            ...
            <a href="https://github.com/pallaviXD/NutriMind-Ai" target="_blank" rel="noopener noreferrer" className="hover:text-foreground transition-colors">GitHub</a>
          </div>
